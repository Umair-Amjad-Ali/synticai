        ...
        >
          <h2 className="text-2xl md:text-3xl lg:text-4xl font-bold text-[#0A051E] mb-4">
            <span className="inline-block border-l-4 border-[#1C75BC] pl-4">
              ...
              <span className="text-gray-400 font-medium">– Take theirs!</span>
            </span>
          </h2>
          <p className="text-gray-500 text-[15px] md:text-[16px] max-w-3xl leading-relaxed">
            Discover how SynticAI has transformed businesses through our
            innovative AI solutions and dedicated partnership.
          </p>
        </motion.div>
              ...
              {getVisibleTestimonials().map(({ item, position }) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div
                      className={`w-full h-full min-h-[350px] p-5 md:p-6 transition-colors duration-500 flex flex-col ${isActive ? "bg-linear-to-br from-[#1C75BC] to-[#0A051E] text-white" : "bg-white text-[#0A051E] border border-gray-100"}`}
                    >
                      {/* Active Card Accent */}
                      {isActive && (
                      ...
                      <div className="relative z-10 flex flex-col h-full text-left">
                        ...
                        <div className="flex items-center gap-3 mb-4">
                          <div
                            className={`w-12 h-12 rounded-full border-2 overflow-hidden shadow-sm shrink-0 ${isActive ? "border-white/30" : "border-gray-200"}`}
                          >
                            <img
                              src={item.image}
                              alt={item.name}
                          ...
                          <div>
                            <h4
                              className={`font-bold text-base ${isActive ? "text-white" : "text-[#0A051E]"}`}
                            >
                              {item.name}
                            </h4>
                            <p
                              className={`text-[12px] font-semibold ${isActive ? "text-[#45d1f5]" : "text-[#1C75BC]"}`}
                            >
                              {item.title}
                            </p>
                          </div>
                        ...
                        <div className="mb-4">
                          <p
                            className={`text-[10px] uppercase font-bold tracking-wider ${isActive ? "text-white/60" : "text-gray-400"}`}
                          >
                            Project
                          </p>
                          <h5
                            className={`text-[15px] font-bold leading-tight ${isActive ? "text-white/90" : "text-gray-800"}`}
                          >
                            {item.project}
                          </h5>
                        </div>
                        ...
                        <div className="grow">
                          <p
                            className={`text-[13px] leading-relaxed italic ${isActive ? "text-white/80" : "text-gray-500"}`}
                          >
                            "{item.quote}"
                          </p>
                        </div>
